...
#include <open62541/ua_pubsub.h>
#include <open62541/ua_pubsub_networkmessage.h>
...
addReaderGroup(UA_Server *server) {
	UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "addReaderGroup is executed");
    if(server == NULL) {
        return UA_STATUSCODE_BADINTERNALERROR;
    }
...
addDataSetReader(UA_Server *server) {
	UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "addDataSetReader is executed");
    if(server == NULL) {
        return UA_STATUSCODE_BADINTERNALERROR;
    }
...
addSubscribedVariables (UA_Server *server, UA_NodeId dataSetReaderId) {
	UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "addSubscribedVariables is executed");
    if(server == NULL)
        return UA_STATUSCODE_BADINTERNALERROR;
...
static void fillTestDataSetMetaData(UA_DataSetMetaDataType *pMetaData) {
	UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "fillTestDataSetMetaData is executed");
    if(pMetaData == NULL) {
        return;
    }
